...
export const addItemToCart = (cartItems, newItem) => {
    ...
    if (exsitingitem) {
        try {
            ...
            item.qty = '+1';
            axios.post('https://quiet-hollows-79620.herokuapp.com/api/cart/updateqty', item, config);
        } catch (err) {
            console.log('Error');
        }
    ...
    try {
        axios.post('https://quiet-hollows-79620.herokuapp.com/api/cart', newItem, config);
    } catch (err) {
        console.log('error: ' + err);
    }

    return [...cartItems, { ...newItem, qty: parseInt(newItem.qty) }];
};

export const reduceQuantity = (cartItems, cartItemToReduce) => {
    return cartItems.map((cartItem) =>
        cartItem.product === cartItemToReduce.product
            ? { ...cartItem, qty: cartItem.qty - 1 }
            : cartItem
    );
};
...
export const reduceCheckoutQuantity = (cartItems, cartItemToReduce) => {
    return cartItems.map((cartItem) =>
        cartItem.product._id === cartItemToReduce.product
            ? { ...cartItem, qty: cartItem.qty - 1 }
            : cartItem
    );
};
