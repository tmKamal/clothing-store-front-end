				...
				<Fragment>
					...
					<div className='product-details'>
						<span className='title'>
							...
							{product.qty <= 30 ? <span className='hot-product'>HOT ITEM</span> : ''}
						</span>

						<span className='review-count'>
							{' '}
							{product.rating.length === 0 ? 'No reviews yet' : `${product.rating.length} reviews`}{' '}
						</span>
						<hr />
						...
						<span className='price'>
							{product.discount ? (
								<span>
									{' '}
									Discounted price <span className='cutprice'>LKR {product.price}</span>
								</span>
							) : (
								''
							)}{' '}
							<span className={product.discount ? 'newprice' : ''}>
								LKR {product.discount ? product.price - product.discount : product.price}
							</span>
						</span>
						<hr />
						<span className='size'>
