...
export const getProducts = (catId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`https://quiet-hollows-79620.herokuapp.com/api/product/category/${catId}`);
    dispatch({
      type: GET_PRODUCTS,
      payload: res.data,
    });
...
export const getProduct = (prodId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`https://quiet-hollows-79620.herokuapp.com/api/product/${prodId}`);
    dispatch({
      type: GET_PRODUCT,
