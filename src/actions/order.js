...
export const placeOrder = (prods, address, payment) => async (dispatch) => {
    try {
        ...
        const res = await axios.post('http://localhost:9000/api/order/', body, config);
        dispatch({
            type: ORDER_PROCESSING,
            payload: 2
        });
...
export const getOrderedItems = () => async (dispatch) => {
    try {
        const res = await axios.get('http://localhost:9000/api/order/items');
        dispatch({
            type: SET_ORDER_ITEMS,
            payload: res.data.orders
        });
...
export const setOrderComplete = (orderId) => async (dispatch) => {
    try {
        ...
        const res = await axios.post('http://localhost:9000/api/order/complete/', body, config);
        dispatch({
            type: SET_ORDER_ITEMS,
            payload: res.data.orders
        });
...
export const itemReviewd = (pid, oid) => async (dispatch) => {
    try {
        ...
        const res = await axios.post('http://localhost:9000/api/order/rated', body, config);
        dispatch({
            type: SET_ORDER_ITEMS,
            payload: res.data.orders
        });
